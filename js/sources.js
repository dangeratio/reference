...
var source_types = Object.keys(sources);
	...
	for(var key in sources){
		for(var i=0; i<sources[key].length; i++) {
			for (var z=0; z<github_filter.length; z++) {
				sources[key][i] = sources[key][i].replace(github_filter[z][0],github_filter[z][1]);
			}
		}
	}
}
...
//
// old version, single type
//
// function process_sources() {
// 	for (var i=0; i<sources.length; i++) {
// 		for (key in sources[i]) {
// 			for (var z=0; z<github_filter.length; z++) {
// 				sources[i][key] = sources[i].replace(github_filter[z][0],github_filter[z][1]);
// 			}
// 		}
// 	}
// }
