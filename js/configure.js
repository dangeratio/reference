...
function left(msg) {
	$("#left").innerHTML = $("#left").innerHTML + msg;
}

function right(msg) {
	$("#right").innerHTML = $("#right").innerHTML + msg;
}
...
function clear_left() {
	$("#left").empty();
	$("#left").append("<div class='button all-source-button'><b>Add All</b><span class='glyphicon glyphicon-circle-arrow-right arrow-span'></span></div>");
}
...
function add_new_source(friendly_name) {
	...
	$("#left").append("<div id='"+friendly_name+"' class='source-button'>" + friendly_name + "<span class='glyphicon glyphicon-circle-arrow-right arrow-span'></span></div><br>");
}
...
function refresh_right_view() {
	...
	for (var i=0; i<local_array.length; i++) {
		var friendly_name = local_array[i].friendly_name;
		$("#right").append("<div id='"+friendly_name+"' class='local-source-button'>" + friendly_name + "<span class='glyphicon glyphicon-remove arrow-span'></span></div><br>");
	}
}
...
$(function() {
	...
	$('body').on('click', '#cancel', function(e) { window.location.replace("reference.html"); });
	...
	//other triggers
	$('body').on('click', '.source-button', function() { add_source(this); });
	$('body').on('click', '.local-source-button', function() { remove_source(this); });
	$('body').on('click', '.all-source-button', function() { add_all_sources(); });
